...
IK_ellipse_init = Robot.ikine(transl(x_ellipse(1),y_ellipse(1), 0),'q0',[-pi/2 0 0],'mask',[1 1 1 0 0 0])'; % Mask vector (1x6 - xyzrpy) specifies the Cartesian DOF that will be ignored in reaching IK solution
...
for i=1:3*360
    % Compute Jacobian
    J(:,:,i) = [L2*cos(theta_ellipse(2,i)), L3*cos(theta_ellipse(3,i));
                L2*sin(theta_ellipse(2,i)), L3*sin(theta_ellipse(3,i))];
    % Compute Jacobian Pseudoinverse
    J_inv(:,:,i) = pinv(J(:,:,i));
    % Compute Joint Velocities
    q_dot = J_inv(:,:,i) * X_dot_ellipse(:,i);
    q_dot_ellipse(:,i) = [0; q_dot];
...
theta_circle = [];
% Initial Joint Angles
IK_circle_init = Robot.ikine(transl(x_circle(1),y_circle(1), 0),'q0',[3*pi/4 0 0],'mask',[1 1 1 0 0 0])'; % Mask vector (1x6 - xyzrpy) specifies the Cartesian DOF that will be ignored in reaching IK solution
...
for i=1:3*360
    % Compute Jacobian
    J(:,:,i) = [L2*cos(theta_circle(2,i)), L3*cos(theta_circle(3,i));
                L2*sin(theta_circle(2,i)), L3*sin(theta_circle(3,i))];
    % Compute Jacobian Pseudoinverse
    J_inv(:,:,i) = pinv(J(:,:,i));
    % Compute Joint Velocities
    q_dot = J_inv(:,:,i) * X_dot_circle(:,i);
    q_dot_circle(:,i) = [0; q_dot];
    % Compute Joint Angles using Euler Forward
...
xlabel("X (m)")
ylabel("Y (m)")
title('Augmented Task-Space Method')
