...
IK_ellipse_init = Robot.ikine(transl(x_ellipse(1),y_ellipse(1), 0),'q0',[-pi/6 0 0],'mask',[1 1 1 0 0 0])'; % Mask vector (1x6 - xyzrpy) specifies the Cartesian DOF that will be ignored in reaching IK solution
...
theta_circle = [];
% Initial Joint Angles
IK_circle_init = Robot.ikine(transl(x_circle(1),y_circle(1), 0),'q0',[pi/2 0 0],'mask',[1 1 1 0 0 0])'; % Mask vector (1x6 - xyzrpy) specifies the Cartesian DOF that will be ignored in reaching IK solution
...
subtitle('$\dot{\theta_2}+\dot{\theta_3}=0$','Interpreter','latex')
legend
grid on
grid minor
xlim([-1,2])
