...
for i=1:3*360
    ...
    neg_grad_V = [-2*theta_ellipse(1,i); - 0.5*theta_ellipse(2,i); -1.32*theta_ellipse(3,i)];
    % Compute Jacobian Pseudoinverse
    J_inv(:,:,i) = pinv(J(:,:,i));
    % Compute Joint Velocities
    q_dot_ellipse(:,i) = (J_inv(:,:,i) * X_dot_ellipse(:,i)) + ((eye(3)-(J_inv(:,:,i)*J(:,:,i)))*neg_grad_V);
...
for i=1:3*360
    ...
    % Artificial Potential
    neg_grad_V = [-2*theta_circle(1,i); - 0.5*theta_circle(2,i); -1.32*theta_circle(3,i)];
    % Compute Jacobian Pseudoinverse
    J_inv(:,:,i) = pinv(J(:,:,i));
    % Compute Joint Velocities
    q_dot_circle(:,i) = (J_inv(:,:,i) * X_dot_circle(:,i)) + ((eye(3)-(J_inv(:,:,i)*J(:,:,i)))*neg_grad_V);
    % Compute Joint Angles using Euler Forward
...
xlabel("X (m)")
ylabel("Y (m)")
title('Artificial Potential Method')
subtitle('$V={\theta_1}^2+0.25*{\theta_2}^2+0.66*{\theta_3}^2$','Interpreter','latex')
legend
grid on
grid minor
xlim([-1,2])
